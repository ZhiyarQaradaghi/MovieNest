...
app.post('/add_movie', (req, res) => {
    const { mov_title, mov_description, mov_rel_year, mov_genre, directors, cast } = req.body;

    db.run(
        `INSERT INTO movie (mov_title, mov_description, mov_rel_year, mov_genre) VALUES (?, ?, ?, ?)`,
        [mov_title, mov_description, mov_rel_year, mov_genre],
        function (err) {
            if (err) {
            ...
            directors.forEach(director => {
                db.run(
                    ...
                    [director.dir_fname, director.dir_lname, director.dir_age, director.dir_country],
                    function (err) {
                        if (err) {
                            console.error(err.message);
